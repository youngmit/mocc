...
std::vector<std::pair<real_t, real_t>> GenGauss(int n_polar)
{
    ...
    while (max(abs(y - y0)) > REAL_FUZZ) {
        ...
        for (int k = 1; k < N1; k++) {
            for (int m = 0; m < N1; m++) {
                L(m, k + 1) =
                    ((2 * k + 1) * y(m) * L(m, k) - k * L(m, k - 1)) / (k + 1);
            }
        }

        for (int i = 0; i < N1; i++) {
            Lp(i) =
                N2 * (L(i, N1 - 1) - y(i) * L(i, N2 - 1)) / (1 - y(i) * y(i));
        }
...
std::vector<Angle> GenProduct(const std::vector<std::pair<real_t, real_t>> &azi,
                              const std::vector<std::pair<real_t, real_t>> &pol)
{
    ...
    for (int i = 0; i < n_azimuthal; i++) {
        for (int j = 0; j < n_polar; j++) {
            Angle angle(azi[i].first, pol[j].first,
