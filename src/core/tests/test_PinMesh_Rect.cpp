...
TEST(test_rect) {
    ...
    CHECK_EQUAL(17, pm->find_reg(Point2(0.0, 0.63), Direction(5.0*PI/4.0, HPI)));
    
    // on bottom edge, pointing in
    CHECK_EQUAL(3, pm->find_reg(Point2(0.252, -0.63), Direction()));
    // on bottom edge, pointing out
    CHECK_EQUAL(-1, pm->find_reg(Point2(0.252, -0.63), Direction(5.0*PI/4.0, HPI)));

    // on right edge, pointing out
    CHECK_EQUAL(-1, pm->find_reg(Point2(0.63, 0.0), Direction()));
    // on bottom edge, pointing in
    CHECK_EQUAL(14, pm->find_reg(Point2(0.63, 0.0), Direction(3.0*PI/4.0, HPI)));
    CHECK_EQUAL(9, pm->find_reg(Point2(0.63, 0.0), Direction(5.0*PI/4.0, HPI)));
