...
public:
    ...
    XSMeshRegion(const VecI &fsrs, real_t *xstr, real_t *xsnf, real_t *xsch,
                 real_t *xsf, real_t *xsrm, const ScatteringMatrix &xssc);
    ...
    VecF reaction_cdf(int ig) const
    {
        ...
        cdf[(int)Reaction::FISSION] =
            cdf[(int)Reaction::SCATTER] + xsmacf_[ig] * scale;
    ...
    bool operator==(const XSMeshRegion &other) const
    {
        ...
        for (int ig = 0; ig < this->n_group(); ig++) {
            if (!fp_equiv_ulp(this->xsmactr(ig), other.xsmactr(ig))) {
                return false;
            }
            if (!fp_equiv_ulp(this->xsmacnf(ig), other.xsmacnf(ig))) {
                return false;
            }
            if (!fp_equiv_ulp(this->xsmacf(ig), other.xsmacf(ig))) {
                return false;
            }
            if (!fp_equiv_ulp(this->xsmacrm(ig), other.xsmacrm(ig))) {
                return false;
            }
    ...
    friend std::ostream &operator<<(std::ostream &os, const XSMeshRegion &xsr);
