...
std::ostream &operator<<(std::ostream &os, const AngularQuadrature &angquad)
{
    const int w = 12;
    os << std::setw(w) << "Alpha" << std::setw(w) << "Theta" << std::setw(w)
       << "omega x" << std::setw(w) << "omega y" << std::setw(w) << "omega z"
       << std::setw(w) << "weight" << std::setw(w) << "rsintheta" << std::endl;
...
{
    VecF alpha;
    alpha.reserve(this->ndir());
    VecF theta;
    theta.reserve(this->ndir());
    VecF ox;
    ox.reserve(this->ndir());
    VecF oy;
    oy.reserve(this->ndir());
    VecF oz;
    oz.reserve(this->ndir());
    VecF w;
    w.reserve(this->ndir());
...
void AngularQuadrature::update_chebyshev_weights()
{
    ...
    for (auto angle_it = this->octant(1); angle_it != this->octant(2);
         ++angle_it) {
    ...
    azi_angles.erase(
        std::unique(azi_angles.begin(), azi_angles.end(), fp_equiv_ulp),
        azi_angles.end());
